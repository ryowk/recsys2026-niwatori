#!/usr/bin/env python3
"""Vectorized builder for the union LambdaRank base feature schema."""
...
def _codes(
    candidate_values: list[str], history_values: list[str]
...
def make_fast_context(features: Any, encoder: Any) -> FastFeatureContext:
    ti = encoder.track_index
    missing = features.MISSING_CAT
    ...
    artist_code_candidate, artist_code_history, n_artist = _codes(
        artist_candidate, artist_history
    )
    album_code_candidate, album_code_history, n_album = _codes(
        album_candidate, album_history
    )
...
def _token_overlap(
    ...
    return (
        np.asarray(rows[:, token_ids].sum(axis=1))
        .ravel()
...
def _history_tag_overlap(
    ...
    hist_counts = (
        np.asarray(
            ctx.track_tag_csr[np.asarray(history_indices, dtype=np.int32)].sum(axis=0)
        )
        .ravel()
        .astype(np.float32, copy=False)
    )
...
def _count_by_code(
    history_codes: np.ndarray, n_codes: int, candidate_codes: np.ndarray
) -> np.ndarray:
...
def build_feature_matrix_fast(
    ...
    ctx = (
        ...
        else make_fast_context(features, encoder)
    )
    ti = encoder.track_index
    ...
    dense_only_all = np.zeros_like(rank_all, dtype=np.float32)

    out_pos = 0
    for start in range(0, len(examples), chunk_examples):
        end = min(start + chunk_examples, len(examples))
        ex_chunk = examples[start:end]
        goal_texts = [features.goal_text() for _ in ex_chunk]
        conv_texts = [features.conversation_text(ex, ti) for ex in ex_chunk]
        query_texts = [features.query_text(ex.user_query) for ex in ex_chunk]
        ...
        for local_i, ex in enumerate(ex_chunk):
            ...
            if len(positions) == 0:
                continue
            cand_idx = candidates.indices[global_i, positions].astype(
                np.int32, copy=False
            )
            m = len(cand_idx)
            ...
            sims = (
                sparse.vstack(
                    [goal_vecs[local_i], conv_vecs[local_i], query_vecs[local_i]]
                )
            ...
            history_indices = [
                ti.id_to_idx[tid] for tid in history_tracks if tid in ti.id_to_idx
            ]
            ...
            if (
                ...
                and ti.dense_emb is not None
                and len(history_indices)
            ):
                ...
                if hdn > 0:
                    ...
                    query_history_dense_cos = float(
                        query_dense_vec @ hist_dense_centroid
                    )
                else:
            ...
            context_tokens = features.tokens(
                features.goal_text() + " " + features.conversation_text(ex)
            )
            profile = ex.user_profile
            # GPA is unavailable in Blind B, so these schema columns remain zero.
            prior: list[str] = []
            ...
            culture_tokens = features.tokens(
                str(profile.get("preferred_musical_culture") or "")
            )
            country_tokens = features.tokens(str(profile.get("country_name") or ""))
            lang_tokens = features.tokens(str(profile.get("preferred_language") or ""))
            ...
            hist_artist_codes = (
                ctx.artist_code_history[history_arr]
                if len(history_arr)
                else np.empty(0, dtype=np.int32)
            )
            hist_album_codes = (
                ctx.album_code_history[history_arr]
                if len(history_arr)
                else np.empty(0, dtype=np.int32)
            )
            hist_tag_codes = (
                ctx.primary_tag_code_history[history_arr]
                if len(history_arr)
                else np.empty(0, dtype=np.int32)
            )
            ...
            row[:, 10] = _count_by_code(
                hist_artist_codes, ctx.n_artist_codes, cand_artist_codes
            )
            row[:, 11] = _count_by_code(
                hist_album_codes, ctx.n_album_codes, cand_album_codes
            )
            if len(history_arr):
                row[:, 12] = np.isin(cand_idx, history_arr, assume_unique=False).astype(
                    np.float32
                )
            else:
                row[:, 12] = 0.0
            ...
            row[:, 22] = _count_by_code(
                hist_tag_codes, ctx.n_primary_tag_codes, cand_tag_codes
            )
            row[:, 23] = _history_tag_overlap(
                ctx, cand_idx, history_indices, cand_tag_rows
            )
            if len(history_indices):
                last_idx = history_indices[-1]
                row[:, 24] = (
                    cand_artist_codes == ctx.artist_code_history[last_idx]
                ).astype(np.float32)
                row[:, 25] = (
                    cand_album_codes == ctx.album_code_history[last_idx]
                ).astype(np.float32)
            else:
                row[:, 24] = 0.0
                row[:, 25] = 0.0
            ...
            if len(history_indices) and history_year_mean > 0:
                ...
                row[:, 27] = np.where(
                    cand_year > 0, np.abs(cand_year - history_year_mean), 0.0
                )
            else:
                row[:, 27] = 0.0
            ...
            row[:, 34:39] = 0.0
            row[:, 39] = encoder.encode_cat(
                "goal_category", example_cats["goal_category"]
            )
            row[:, 40] = encoder.encode_cat(
                "goal_specificity", example_cats["goal_specificity"]
            )
            row[:, 41] = encoder.encode_cat(
                "latest_goal_progress", example_cats["latest_goal_progress"]
            )

            if y is not None:
                gold_idx = ti.id_to_idx.get(ex.gold_track_id or "")
                y[sl] = (
                    (cand_idx == gold_idx).astype(np.int8)
